...
关于下载交通网络，目前只能下载 SUMO 网络。UXsim 网络可以从现有的 SUMO 网络转换而来。

关于编辑交通网络，SUMO 网络需要使用 NetEdit，而 UXsim 网络可以在 V2Sim 图形用户界面中直接编辑。

配电网必须手动创建，并可以在 V2Sim 图形用户界面中编辑。

## 下载 SUMO 交通网络
运行以下命令打开 SUMO 的交通网络下载器：
```
v2sim-osm
```
...
+ 您可以根据偏好调整 `Options` 项。
  ...
  - 推荐启用 `Car-only Network`，因为 V2Sim 不模拟车辆以外的对象。
+ 在第二个标签页中，默认仅勾选 `highway`。不推荐其他选项，因为 V2Sim 不会模拟它们的运动。
+ 适当配置后点击 `Generate Scenario` 按钮。案例将在 `cases` 文件夹中生成，名称使用当前时间。案例生成后，V2Sim GUI 将自动打开。它看起来像下图：<br> ![alt text](../imgs/0.png)
+ 如果生成的案例未被自动选中，点击 `Add project...` 链接，选择您创建的文件夹，然后打开它。您将看到类似这样的内容：<br> ![alt text](../imgs/3.png)

## 将 SUMO 网络转换为 UXsim 网络
在上一节中，我们为 SUMO 下载了道路网络。但是，如果我们想使用 UXsim 作为交通仿真器，我们需要转换案例。
